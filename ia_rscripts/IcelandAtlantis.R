...
pdf(file = "study_map_17sept2014.pdf")
ggplot(data=atlantis.map.tmp, aes(long, lat, group=group)) + geom_polygon(aes(fill = island)) + geom_path() + theme_bw() + xlab("Longitude") + ylab("Latitude") + coord_map() + scale_fill_manual(values = c("#6699FF","white", "orangered4")) + theme(legend.position="none")  + scale_y_continuous(breaks=seq(58, 76, 1)) + scale_x_continuous(breaks =seq(-50,0,2))
dev.off()
